...
class AFNONet(nn.Module):
    def __init__(
        ...
        self.patch_embed = PatchEmbed(img_size=img_size, patch_size=self.patch_size, in_chans=self.in_chans, embed_dim=self.embed_dim)
        ...
        self.blocks = nn.ModuleList([
            Block(dim=self.embed_dim, mlp_ratio=mlp_ratio, drop=drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
...
if __name__ == "__main__":
    model = AFNONet(img_size=(4,4), patch_size=(2,2), in_chans=9, out_chans=9)
    sample = torch.randn(1, 9, 4,4)
    result = model(sample)
    print(result.shape)
